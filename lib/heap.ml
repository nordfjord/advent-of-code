...
module Make (X : Ordered) = struct
  ...
  let copy h =
    { size = h.size; data = Array.copy h.data; dummy = h.dummy; min_cap = h.min_cap }
  ...
  let remove_and_add h x = if h.size = 0 then add h x else movedown h.data h.size 0 x
  ...
  let to_sequence_mutable h =
    let open Base in
    Sequence.unfold ~init:h ~f:(fun h ->
      if is_empty h then None else Some (pop_minimum h, h))

  let to_sequence h =
    let h' = copy h in
    to_sequence_mutable h'
end
