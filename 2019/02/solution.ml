...
type operation = Add of int * int * int | Mul of int * int * int | Halt
...
let get_op program i =
  ...
  | 99 ->
      Halt
  | _ ->
      failwith ("Invalid opcode " ^ string_of_int program.(i))
...
let get_program () =
  Prelude.Aoc.stdin_seq () |> List.of_seq |> List.hd |> String.split_on_char ','
...
let part1 () =
  ...
  program.(1) <- 12 ;
  program.(2) <- 2 ;
  run_program program 0 |> printf "Part 1: %d\n"
...
let part2 () =
  ...
  find_target program target 0 0 |> printf "Part 2: %d\n"
