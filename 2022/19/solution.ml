...
let parse_blueprint s =
  Scanf.sscanf
    ...
    "Blueprint %d: Each ore robot costs %d ore. Each clay robot costs %d ore. Each \
     obsidian robot costs %d ore and %d clay. Each geode robot costs %d ore and %d \
     obsidian."
...
let actions = [ BuildOreRobot; BuildClayRobot; BuildObsidianRobot; BuildGeodeRobot ]
...
let next_states bp state =
  let next_actions =
    ...
    |> List.filter_map (fun action ->
      if action = BuildOreRobot && state.robots.ore >= bp.max_ore
      then None (* We don't want more clay than necessary *)
      else if action = BuildClayRobot && state.robots.clay >= bp.max_clay
      then None (* We don't want more obsidian than necessary *)
      else if action = BuildObsidianRobot && state.robots.obsidian >= bp.max_obsidian
      then None
...
let rec simulate (blueprint : blueprint) max_geodes state =
  ...
  then ()
  else if potential_production state <= !max_geodes
  then ()
