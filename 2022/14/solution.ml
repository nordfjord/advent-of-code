...
let lines =
  ...
  |> Seq.map (fun line ->
    ...
    |> List.map (fun segment -> Scanf.sscanf segment "%d,%d" (fun col row -> (col, row))))
  |> List.of_seq
...
let initial : paths = Array.init max_row (fun _ -> Array.init max_col (fun _ -> Air))
