...
let is_free tbl (row, col) move =
  (match move with
   | Up -> [ (row - 1, col); (row - 1, col - 1); (row - 1, col + 1) ]
   | Down -> [ (row + 1, col); (row + 1, col - 1); (row + 1, col + 1) ]
   | Left -> [ (row, col - 1); (row + 1, col - 1); (row - 1, col - 1) ]
   | Right -> [ (row, col + 1); (row + 1, col + 1); (row - 1, col + 1) ])
...
let propose_move tbl dirs pos =
  ...
  match results with
  | [] | [ _; _; _; _ ] -> None
  | dir :: _ -> Some (pos, move pos dir)

let count f = Array.fold_left (fun sum v -> if f v then sum + 1 else sum) 0

let simulate tbl dirs =
  let proposed_moves =
    tbl |> Hashtbl.to_seq_keys |> Seq.filter_map (propose_move tbl dirs) |> Array.of_seq
  in
  let unique_moves =
    proposed_moves
    |> Array.to_seq
...
let get_bounds tbl =
  ...
  |> Hashtbl.to_seq_keys
  |> Seq.fold_left
       (fun ((r1, c1), (r2, c2)) (r, c) -> ((min r1 r, min c1 c), (max r2 r, max c2 c)))
