...
let rec dfs max_pressure unvisited pressure time (i, distance) =
  ...
  in
  if next_time < 0
  then max_pressure := max !max_pressure pressure
  else if List.length next_nodes = 0
  then max_pressure := max !max_pressure next_pressure
  else
    List.iter
      (fun (node, dist) ->
       ...
       (fun max_pressure (left_valves, left_pressure) ->
         ...
         |> Array.fold_left
              (fun max_pressure (right_valves, right_pressure) ->
                ...
                if pressure > max_pressure && IntSet.disjoint left_valves right_valves
                then pressure
                else max_pressure)
              max_pressure)
       0
