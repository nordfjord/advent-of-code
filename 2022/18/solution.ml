...
let points =
  Array.init size (fun _ -> Array.init size (fun _ -> Array.make size Air))
...
let rec expand_airpocket_aux to_visit visited =
  ...
  | point :: to_visit ->
      ...
      if not (is_in_bounds point) then PointSet.empty
        (* can't go further, it's lava *)
      else if is_lava point || PointSet.mem point visited then
        expand_airpocket_aux to_visit visited
      else
  ...
  for x = 0 to 20 do
    for y = 0 to 20 do
      for z = 0 to 20 do
        ...
        if is_air && has_adjacent_lava then
          air_pockets :=
