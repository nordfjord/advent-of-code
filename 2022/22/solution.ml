...
let rows, cols =
  (Array.length raw_map, raw_map |> Array.map String.length |> Array.fold_left max 0)
...
let score row col heading = (1000 * (row + 1)) + (4 * (col + 1)) + int_of_heading heading
...
let () =
  ...
  in
  printf "\n";
  printf "face=%d; row=%d; col=%d; heading=%d\n" face row col (int_of_heading heading);
