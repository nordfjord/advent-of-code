...
let adjust_floor height floor rock col row =
  ...
  then (height, floor)
    ...
    let floor =
      Array.append floor (Array.make (max 0 (row + List.length rock - current_height)) 0)
    in
    rock |> List.iteri (fun i r -> floor.(row + i) <- floor.(row + i) lor (r lsl col));
...
let simulate (width, rock) jets (jet_idx, height, floor) =
  ...
  let update_col shift =
    ...
    if 0 <= new_col && new_col <= 7 - width && is_falling rock floor !row new_col
    then col := new_col
  in
...
let part2 () =
  ...
  let jet_idx, height, floor = part2_solution rock_seq input 1_000_000_000_000 in
  printf "%d, %d\n" jet_idx height;
  print_floor floor;
  printf "Part 2: %d\n" (Array.length floor + height)
